...
- Primary product surfaces: Scenara Console 的 14 个工作区、图片/视频/文档/实时流解析工作台、全局导航、全局外壳中的连接设置对话框和各页管理对话框。
- Evidence reviewed: `README.md`、`docs/brand/品牌规范.md`、`docs/strategy/媒体解析产品标准.md`、`frontend/console/src/App.vue`、`frontend/console/src/router.ts`、`frontend/console/src/styles.css`、`frontend/console/src/labels.ts`、全部 `frontend/console/src/views/*.vue`、Vitest 与 Playwright 验收用例。
...
- Success signals: 全部工作区的通用界面文案均为中文；三类媒体均可在解析工作台完成任务；枚举和后端英文说明不直接暴露；桌面与移动端无溢出或交互回归；任务状态、采样时间点、媒体元数据和失败原因可追溯。
...
- Core routes/screens: 总览、人像解析、OCR 文档解析、数据资产、解析结果、运行历史、智能检索、长期轨迹、布控预警、数据集治理、领域与能力、模型生命周期、反馈与难例、接入与权限、系统运维、审计中心。`/pipelines` 重定向到`/capabilities`，`/enterprise` 与 `/governance` 重定向到系统运维；各领域解析页共享图片/视频/文档/实时流输入模式和运行观察模式。
...
- Test/screenshot expectations: Vitest 静态扫描阻止通用英文回归；Playwright 覆盖全部工作区路由、桌面和移动视口、三类媒体模式、运行状态、结果浏览、标签页及一次性凭据流程。
