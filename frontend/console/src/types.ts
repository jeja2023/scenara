...
export interface Organization {
  tenant_id: string; display_name: string; created_at: number; updated_at: number;
}
export interface Project {
  tenant_id: string; project_id: string; display_name: string; created_at: number; updated_at: number;
}
export interface UserAccount {
  tenant_id: string; user_id: string; display_name: string; email?: string | null;
  disabled: boolean; created_at: number; updated_at: number;
}
export interface Role {
  tenant_id: string; role_id: string; display_name: string; scopes: string[]; product_ids: string[];
  created_at: number; updated_at: number;
}
export interface Membership {
  tenant_id: string; project_id: string; principal_id: string; principal_type: "user" | "service_account";
  role_ids: string[]; created_at: number; updated_at: number;
}
export interface ServiceAccount {
  tenant_id: string; project_id: string; service_account_id: string; display_name: string;
  scopes: string[]; product_ids: string[]; disabled: boolean; created_at: number; updated_at: number;
}
export interface ApiKeyRecord {
  tenant_id: string; project_id: string; key_id: string; service_account_id: string; name: string;
  token_prefix: string; scopes: string[]; product_ids: string[]; expires_at?: number | null;
  revoked_at?: number | null; last_used_at?: number | null; created_at: number;
}
export interface ProductEntitlement {
  tenant_id: string; project_id: string; product_id: string; status: "active" | "suspended";
  source: "manual" | "enterprise_license" | "system"; created_at: number; updated_at: number;
}
export interface IamInventory {
  organizations: number; projects: number; users: number; roles: number; memberships: number;
  service_accounts: number; api_keys: number; product_entitlements: number;
}
export interface IamSummary {
  schema_version: "1.0"; tenant_id: string; project_id: string;
  inventory: IamInventory; default_admin_scopes: string[];
}
