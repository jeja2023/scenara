...
async function refresh(): Promise<void> {
  ...
  error.value = "";
  try {
    [runs.value, domains.value, pipelines.value, products.value, repositoryTopology.value, portraitIntelligence.value] =
      await Promise.all([
  ...
  <section class="page">
    ...
    <section v-if="portraitIntelligence" class="panel portrait-intelligence">
      <div class="panel-header">
        <div>
          <h2>人像智能基础平台</h2>
          <p>六大核心模块、三项战略资产与七项能力就绪度。内容反映当前成熟度意图，不代表已部署的模型质量。</p>
        </div>
        <span class="badge">契约 {{ portraitIntelligence.schema_version }}</span>
      </div>
      ...
      <div class="pi-modules">
        ...
        >
          <div class="pi-card-header">
            ...
            <span class="badge" :class="`pi-${mod.maturity}`">{{ labelPortraitMaturity(mod.maturity) }}</span>
          </div>
          <p>{{ labelPortraitModuleSummary(mod.module_id) }}</p>
          <small class="pi-gate">{{ labelPortraitModuleGate(mod.module_id) }}</small>
        </article>
      </div>
      ...
      <div class="pi-capabilities">
        <div class="pi-cap-header">
          <strong>能力就绪度</strong>
          <span class="badge">
            {{ portraitIntelligence.capabilities.filter((c) => c.readiness === "ready").length }} /
        ...
        <div class="pi-cap-grid">
          <div
            ...
            class="pi-cap-row"
          >
            <span class="pi-cap-name">{{ labelPortraitCapability(cap.capability_id) }}</span>
            <span class="badge" :class="`pi-${cap.readiness}`">{{ labelPortraitReadiness(cap.readiness) }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="pi-assets">
        ...
        >
          <div class="pi-card-header">
            ...
            <span class="badge" :class="`pi-${asset.maturity}`">{{ labelPortraitMaturity(asset.maturity) }}</span>
          </div>
          <p>{{ labelPortraitAssetSummary(asset.asset_id) }}</p>
          <small class="pi-gate">{{ labelPortraitAssetGate(asset.asset_id) }}</small>
        </article>
      </div>
    </section>
...
.portrait-intelligence { margin-bottom: 16px; }
.portrait-intelligence .panel-header > div { display: grid; gap: 4px; }
.portrait-intelligence .panel-header p { margin: 0; color: var(--muted); font-size: 12px; }
.pi-modules { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 12px; padding: 16px; }
.pi-module-card { display: grid; gap: 8px; padding: 14px; border: 1px solid var(--line); border-top: 3px solid #8b6fd4; border-radius: 5px; background: #fff; }
.pi-card-header { display: flex; align-items: flex-start; justify-content: space-between; gap: 10px; }
.pi-card-header strong { min-width: 0; font-size: 13px; overflow-wrap: anywhere; }
.pi-module-card p { margin: 0; color: #45534f; font-size: 12px; line-height: 1.45; }
.pi-gate { color: var(--muted); font-size: 11px; line-height: 1.45; }
.badge.pi-available { background: #e4f2e9; color: #226a42; }
.badge.pi-partial { background: #e8f4fd; color: #1a5a8b; }
.badge.pi-seed { background: var(--teal-soft); color: #08636c; }
.badge.pi-planned { background: #edf0ef; color: #45534f; }
.badge.pi-external { background: #f3f0f9; color: #6b4fa0; }
.badge.pi-ready { background: #e4f2e9; color: #226a42; }
.badge.pi-fallback { background: #fbf0de; color: #8b5a14; }
.badge.pi-placeholder { background: #fde8e8; color: #8b1a1a; }
.badge.pi-not_configured { background: #edf0ef; color: #45534f; }
.pi-capabilities { padding: 0 16px 16px; border-top: 1px solid var(--line); }
.pi-cap-header { display: flex; align-items: center; gap: 10px; padding: 13px 0 10px; }
.pi-cap-header strong { font-size: 13px; }
.pi-cap-grid { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 7px 16px; }
.pi-cap-row { display: flex; align-items: center; justify-content: space-between; gap: 6px; padding: 5px 0; border-bottom: 1px solid var(--line); }
.pi-cap-name { font-size: 12px; color: #25332f; }
.pi-assets { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 12px; padding: 12px 16px 16px; border-top: 1px solid var(--line); }
.pi-asset-card { display: grid; gap: 8px; padding: 14px; border: 1px solid var(--line); border-top: 3px solid var(--amber); border-radius: 5px; background: #fff; }
...
@media (max-width: 1180px) {
  ...
  .pi-modules { grid-template-columns: repeat(2, minmax(0, 1fr)); }
  .pi-cap-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); }
  .pi-assets { grid-template-columns: repeat(2, minmax(0, 1fr)); }
}
...
@media (max-width: 560px) {
  ...
  .pi-modules { grid-template-columns: 1fr; padding: 12px; }
  .pi-cap-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); }
  .pi-assets { grid-template-columns: 1fr; padding: 12px; }
