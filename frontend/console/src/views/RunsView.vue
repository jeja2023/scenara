...
const activeCount = computed(() => runs.value.filter((item) => !["completed", "failed", "cancelled"].includes(item.status)).length);
const failedCount = computed(() => runs.value.filter((item) => item.status === "failed").length);
const completedCount = computed(() => runs.value.filter((item) => item.status === "completed").length);
...
function downloadResult(): void {
  if (!rawResult.value || !selected.value) return;
  const blob = new Blob([rawResult.value], { type: "application/json" });
  const url = URL.createObjectURL(blob);
  const link = document.createElement("a");
  link.href = url;
  link.download = `scenara-${selected.value.run_id}.json`;
  link.click();
...
function duration(run: Run): string {
  ...
  const end = run.completed_at ?? (["completed", "failed", "cancelled"].includes(run.status) ? run.updated_at : Date.now() / 1000);
...
<template>
  <section class="page">
    ...
    <div class="stats">
      <div class="stat teal"><span>全部运行</span><strong>{{ total }}</strong><small>当前筛选条件</small></div>
      <div class="stat"><span>进行中</span><strong>{{ activeCount }}</strong><small>排队、运行或暂停</small></div>
      <div class="stat green"><span>已完成</span><strong>{{ completedCount }}</strong><small>结果可查询</small></div>
      <div class="stat coral"><span>失败</span><strong>{{ failedCount }}</strong><small>需要排查原因</small></div>
    </div>
    ...
    <div class="panel filter-panel">
      ...
      <div class="panel-body">
        <div class="toolbar">
          <select v-model="domain" aria-label="领域筛选" @change="refresh"><option value="">全部领域</option><option value="portrait">人像</option><option value="ocr">OCR 文档</option></select>
          <select v-model="status" aria-label="状态筛选" @change="refresh"><option value="">全部状态</option><option value="queued">排队中</option><option value="running">运行中</option><option value="paused">已暂停</option><option value="completed">已完成</option><option value="failed">失败</option><option value="cancelled">已取消</option></select>
    ...
    <section class="panel runs-panel">
      <div class="table-scroll">
        <table class="data-table">
          <thead><tr><th>运行</th><th>领域</th><th>状态</th><th>进度</th><th>流水线</th><th>资产/源</th><th>耗时</th><th>操作</th></tr></thead>
          <tbody>
            <tr v-for="run in runs" :key="run.run_id">
              <td class="mono truncate">{{ run.run_id }}</td>
              <td>{{ labelDomain(run.domain) }}</td>
              <td>
                <span class="badge" :class="run.status">{{ labelRunStatus(run.status) }}</span>
                <div v-if="run.error_code" class="muted small">{{ labelRunError(run.error_code) }}</div>
                <div v-else-if="run.termination_reason" class="muted small">{{ labelTerminationReason(run.termination_reason) }}</div>
              </td>
              <td class="progress-cell">
                <div class="progress-track" role="progressbar" :aria-valuenow="progressPercent(run)" aria-valuemin="0" aria-valuemax="100"><span :style="{ width: `${progressPercent(run)}%` }" /></div>
    ...
    <dialog :open="!!selected" class="modal result-modal">
      <div class="modal-header">
        <div><h2>{{ selected?.run_id }}</h2><p>{{ selected ? labelRunStatus(selected.status) : "" }}</p></div>
        <div class="toolbar compact">
          <button v-if="rawResult" class="button secondary" @click="downloadResult"><Download :size="15" />下载</button>
          <button class="icon-button" title="关闭" aria-label="关闭" @click="selected = null"><X :size="18" /></button>
        </div>
      </div>
...
.auto-refresh { display: inline-flex; align-items: center; gap: 6px; font-size: 12px; color: var(--muted); }
.small { font-size: 11px; }
.progress-cell { min-width: 92px; }
.progress-track { height: 6px; overflow: hidden; background: #dfe6e3; border-radius: 3px; }
.progress-track span { display: block; height: 100%; background: var(--teal); transition: width .2s ease; }
.progress-cell small { display: block; margin-top: 3px; color: var(--muted); font-size: 11px; }
