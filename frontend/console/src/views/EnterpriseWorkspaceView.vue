...
async function refresh(): Promise<void> {
  ...
  error.value = "";
  try {
    const [nextStatus, nextIncidents, nextCases, nextEvidence] =
      await Promise.all([
    ...
    error.value = unavailable.value
      ? ""
      : userFacingError(caught, "企业数据加载失败，请稍后重试");
  } finally {
    loading.value = false;
  }
}
...
<template>
  <section class="page">
    ...
    <template v-if="status">
      <div class="stats">
        <div class="stat teal">
          <span>许可证</span><strong>{{ status.license_id }}</strong
          ><small>{{ status.customer }}</small>
        </div>
        <div class="stat">
          <span>支持等级</span
          ><strong>{{ labelSupportTier(status.support_tier) }}</strong
          ><small>{{ status.entitlements.length }} 项权益</small>
        </div>
        <div class="stat green">
          <span>运行用量</span><strong>{{ status.usage.runs ?? 0 }}</strong
          ><small>上限 {{ status.limits.runs ?? "无限制" }}</small>
        </div>
        <div class="stat coral">
          ...
          ><small class="mono">{{ status.document_sha256.slice(0, 12) }}</small>
        </div>
      </div>
      ...
      <div class="enterprise-grid">
        ...
        <section class="panel">
          ...
          <div class="panel-body">
            <div class="form-grid">
              <label class="span-2"
                ><span>标题</span><input v-model="incidentForm.title"
              /></label>
              <label
                ><span>严重级别</span
                ><select v-model="incidentForm.severity">
                  <option value="sev1">一级</option>
                  <option value="sev2">二级</option>
                  <option value="sev3">三级</option>
                  <option value="sev4">四级</option>
                </select></label
              >
              <label class="span-2"
                ><span>摘要</span
                ><textarea v-model="incidentForm.summary"></textarea>
              </label>
            </div>
            <button
              class="button primary submit"
              :disabled="!incidentForm.title"
        ...
        <section class="panel">
          ...
          <div class="table-scroll">
            <table class="data-table">
              ...
              <tbody>
                ...
                >
                  ...
                  <td>{{ labelCaseStatus(incident.status) }}</td>
                  <td>
            ...
            <div v-if="!incidents.length" class="empty">暂无事件。</div>
          </div>
        </section>
        ...
        <section class="panel">
          ...
          <div class="panel-body">
            <div class="form-grid">
              <label class="span-2"
                ><span>主题</span><input v-model="supportForm.subject"
              /></label>
              <label
                ><span>优先级</span
                ><select v-model="supportForm.priority">
              ...
              <label class="span-2"
                ><span>描述</span
                ><textarea v-model="supportForm.description"></textarea>
              </label>
            </div>
            <button
              class="button primary submit"
              :disabled="!supportForm.subject || !supportForm.description"
              @click="createSupportCase"
            >
              <Plus :size="16" />创建工单
            </button>
            <div class="record-list">
              <div v-for="item in cases" :key="item.case_id">
                ...
                ><span
                  ...
                  {{ labelCaseStatus(item.status) }}</span
                >
              </div>
            </div>
          </div>
        ...
        <section class="panel wide">
          <div class="panel-header">
            <h2>合规证据</h2>
            <span class="badge">{{ evidence.length }}</span>
          </div>
          <div class="panel-body">
          ...
          <div class="table-scroll">
            <table class="data-table">
              ...
              <tbody>
                <tr v-for="item in evidence" :key="item.evidence_id">
                  <td>{{ item.evidence_type }}</td>
                  <td class="truncate">{{ item.object_ref }}</td>
