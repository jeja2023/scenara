...
const error = ref("");
async function refresh(): Promise<void> { try { status.value = await api<Status>("/api/v1/system/status"); } catch (caught) { error.value = userFacingError(caught, "运行状态加载失败，请稍后重试"); } }
onMounted(refresh);
</script>

<template><section class="page"><div class="page-header"><div><h1>运维</h1><p>运行后端与生产门禁状态。</p></div><button class="button secondary" @click="refresh">刷新</button></div><p v-if="error" class="callout error">{{ error }}</p><div v-if="status" class="stats"><div class="stat teal"><span>配置档</span><strong>{{ labelRuntime(status.profile) }}</strong><small>Scenara {{ labelVersion(status.version) }}</small></div><div class="stat"><span>状态存储</span><strong>{{ labelRuntime(status.state_backend) }}</strong><small>唯一事实源</small></div><div class="stat"><span>对象存储</span><strong>{{ labelRuntime(status.object_backend) }}</strong><small>媒体与结果文档</small></div><div class="stat green"><span>队列</span><strong>{{ labelRuntime(status.queue_backend) }}</strong><small>执行投递</small></div></div><section class="panel"><div class="panel-header"><h2>门禁</h2></div><div class="panel-body checks"><div><span>接口认证</span><strong :class="status?.auth_required ? 'ok' : 'warn'">{{ status?.auth_required ? "开启" : "开发关闭" }}</strong></div><div><span>生产模型强制</span><strong :class="status?.production_models_required ? 'ok' : 'warn'">{{ status?.production_models_required ? "开启" : "开发关闭" }}</strong></div><div><span>审计持久化</span><strong class="warn">数据结构已建立</strong></div><div><span>备份恢复证据</span><strong class="warn">待验证</strong></div></div></section></section></template>
