...
try:
    from scripts.code_size_gate import violations as code_size_violations
except ModuleNotFoundError:  # Direct execution from scripts/ remains supported.
    from code_size_gate import violations as code_size_violations
...
REQUIRED_IMPLEMENTATION = (
    ...
    "docs/release/0.3.0-dev.30发布说明.md",
    "docs/release/0.3.0-dev.31发布说明.md",
    "docs/release/0.3.0-dev.32发布说明.md",
    "docs/release/0.3.0-dev.33发布说明.md",
    "docs/release/0.3.0-dev.34发布说明.md",
    "docs/release/0.3.0-dev.39发布说明.md",
    "docs/release/0.3.0-dev.40发布说明.md",
...
PLACEHOLDER = re.compile(
    r"(?:<[^>]+>|\b(?:example|replace|todo|tbd)\b|待填写|占位)", re.IGNORECASE
)
LICENSE_PLACEHOLDER = re.compile(
    r"engineering placeholder|must receive legal review", re.IGNORECASE
)
...
def implementation_errors() -> list[str]:
    errors = [
        f"missing implementation deliverable: {name}"
        for name in REQUIRED_IMPLEMENTATION
        if not (ROOT / name).is_file()
    ]
    errors.extend(code_size_violations(ROOT))
...
def _timestamp(
    raw: Any, name: str, evidence_type: str, errors: list[str]
) -> datetime | None:
...
def _string_set(
    ...
    if not isinstance(value, list) or any(
        ...
        errors.append(
            f"{evidence_type}: metadata.{field} must be a list of non-empty strings"
        )
        return set()
    return set(value)
...
def _required_values(
    metadata: dict[str, Any], names: set[str], evidence_type: str
) -> list[str]:
    return [
        f"{evidence_type}: metadata.{name} is required"
...
def _metadata_errors(evidence_type: str, metadata: dict[str, Any]) -> list[str]:
    ...
    if evidence_type in {
        ...
        if (
            not metadata.get("dataset_version")
            or metadata.get("rights_cleared") is not True
        ):
            errors.append(
                f"{evidence_type}: versioned and rights-cleared dataset evidence is required"
            )
        ...
        if (
            not isinstance(metadata.get("thresholds"), dict)
            or not metadata["thresholds"]
        ):
            errors.append(f"{evidence_type}: fixed evaluation thresholds are required")
        if (
            not isinstance(metadata.get("tolerances"), dict)
            or not metadata["tolerances"]
        ):
        ...
        if fixed_at is not None and fixed_at.utcoffset() is None:
            errors.append(
                f"{evidence_type}: thresholds_fixed_at must include a timezone"
            )
        if metadata.get("thresholds_fixed_before_run") is not True:
            errors.append(
                f"{evidence_type}: thresholds must be fixed before evaluation"
            )
        runs = metadata.get("runs")
        if not isinstance(runs, list) or len(runs) < 2:
            errors.append(
                f"{evidence_type}: metadata.runs must contain at least two run records"
            )
            runs = []
        run_ids: set[str] = set()
        for index, run in enumerate(runs):
            if not isinstance(run, dict):
                errors.append(
                    f"{evidence_type}: metadata.runs[{index}] must be an object"
                )
                continue
            run_id = run.get("run_id")
            if not isinstance(run_id, str) or not run_id or PLACEHOLDER.search(run_id):
                errors.append(
                    f"{evidence_type}: metadata.runs[{index}].run_id is required"
                )
            ...
            if not _valid_sha256(run.get("output_sha256")):
                errors.append(
                    f"{evidence_type}: metadata.runs[{index}].output_sha256 must be a SHA-256"
                )
            if run.get("exit_code") != 0:
                errors.append(
                    f"{evidence_type}: metadata.runs[{index}].exit_code must be zero"
                )
            run_at = _timestamp(
                run.get("executed_at"),
            ...
            if (
                ...
                errors.append(
                    f"{evidence_type}: thresholds must be fixed before every run"
                )
            if not isinstance(run.get("metrics"), dict) or not run["metrics"]:
                errors.append(
                    f"{evidence_type}: metadata.runs[{index}].metrics are required"
                )
        ...
        if isinstance(thresholds, dict) and isinstance(tolerances, dict):
            for threshold_name, threshold_value in thresholds.items():
                ...
                if limit is None:
                    errors.append(
                        f"{evidence_type}: threshold {threshold_name} must be finite"
                    )
                    continue
                ...
                for index, run in enumerate(runs):
                    ...
                    elif (minimum and measured < limit) or (
                        not minimum and measured > limit
                    ):
                        errors.append(
            ...
            for metric_name, tolerance_value in tolerances.items():
                ...
                elif len(values) == len(runs) and all(
        ...
        if metadata.get("within_tolerance") is not True:
            errors.append(
                f"{evidence_type}: two reproducible runs within the fixed tolerance are required"
            )
        command = metadata.get("command")
        if not isinstance(command, str) or not command or PLACEHOLDER.search(command):
            errors.append(
                f"{evidence_type}: reproducible evaluation command is required"
            )
        ...
        if not required <= _string_set(metadata, "scenarios", evidence_type, errors):
            errors.append(
                "gpu_capacity: all capacity and recovery scenarios are required"
            )
        errors.extend(
            _required_values(
                metadata,
        ...
        if p50 is None or p95 is None or p99 is None or not 0 <= p50 <= p95 <= p99:
            errors.append(
                "gpu_capacity: latency percentiles must satisfy 0 <= p50 <= p95 <= p99"
            )
        ...
        for scenario in sorted(required):
            ...
            if not isinstance(result, dict) or result.get("status") != "passed":
                errors.append(
                    f"gpu_capacity: scenario_results.{scenario}.status must be passed"
                )
                continue
            if result.get("exit_code") != 0:
                errors.append(
                    f"gpu_capacity: scenario_results.{scenario}.exit_code must be zero"
                )
        ...
        if not required <= _string_set(metadata, "services", evidence_type, errors):
            errors.append(
                "integration_services: PostgreSQL/pgvector, Redis, and MinIO are required"
            )
        if (
            metadata.get("skipped_tests") != 0
            or metadata.get("duplicate_logical_results") != 0
        ):
            errors.append(
                "integration_services: no skipped tests or duplicate logical results are allowed"
            )
        if metadata.get("redis_rebuild_verified") is not True:
            errors.append(
                "integration_services: Redis rebuild from PostgreSQL and MinIO must be verified"
            )
        ...
        if not required <= _string_set(metadata, "scenarios", evidence_type, errors):
            errors.append(
                "security_assessment: all required security scenarios must be covered"
            )
    elif evidence_type == "model_rights":
        models = metadata.get("models")
        if not isinstance(models, list) or not models:
            errors.append(
                "model_rights: metadata.models must be a non-empty list of model records"
            )
            models = []
        required_fields = {
            "artifact_sha256",
        ...
        for index, model in enumerate(models):
            if not isinstance(model, dict):
                errors.append(
                    f"model_rights: metadata.models[{index}] must be an object"
                )
                continue
            missing = sorted(
                field
            ...
            if not SHA256.fullmatch(str(model.get("artifact_sha256", ""))):
                errors.append(
                    f"model_rights: metadata.models[{index}].artifact_sha256 must be a SHA-256"
                )
            if not _valid_sha256(model.get("rights_record_sha256")):
                errors.append(
                    f"model_rights: metadata.models[{index}].rights_record_sha256 must be a SHA-256"
                )
            key = (str(model.get("model_id", "")), str(model.get("model_version", "")))
            if key in model_keys:
                errors.append(
                    "model_rights: model id and version records must be unique"
                )
            model_keys.add(key)
            for field in (
                ...
                if model.get(field) is not True:
                    errors.append(
                        f"model_rights: metadata.models[{index}].{field} must be true"
                    )
        if metadata.get("all_rights_cleared") is not True or not models:
            errors.append(
                "model_rights: every production model must have cleared rights"
            )
        ...
        if metadata.get("review_basis") != "personal_project_self_review":
            errors.append(
                "software_license: review_basis must identify the personal-project self-review"
            )
        if not required <= _string_set(metadata, "review_scope", evidence_type, errors):
            errors.append(
                "software_license: all license self-review areas are required"
            )
    elif evidence_type == "offline_install":
        required = {"health", "console", "example_clients", "core_parse"}
        if (
            metadata.get("blank_host") is not True
            or metadata.get("isolated_network") is not True
        ):
        ...
        if (
            metadata.get("host_os") != "ubuntu"
            or metadata.get("host_version") != "24.04"
        ):
        ...
        if not SOURCE_COMMIT.fullmatch(str(metadata.get("source_commit", ""))):
            errors.append(
                "offline_install: source_commit must be a full lowercase Git SHA"
            )
        ...
        if not isinstance(services, dict) or any(
            services.get(name) != "running" for name in required_services
        ):
        ...
        for check in sorted(required):
            ...
            if not isinstance(result, dict) or result.get("status") != "passed":
                errors.append(
                    f"offline_install: check_results.{check}.status must be passed"
                )
                continue
            if result.get("exit_code") != 0:
                errors.append(
                    f"offline_install: check_results.{check}.exit_code must be zero"
                )
            if not _valid_sha256(result.get("output_sha256")):
                errors.append(
        ...
        if not required <= _string_set(
            metadata, "entities_verified", evidence_type, errors
        ):
            errors.append(
                "backup_restore: all required business entities must be verified"
            )
    return errors
...
def validate_entry(
    ...
    *,
    ...
    if report is not None and reports_root not in report.parents:
        errors.append(
            f"{evidence_type}: report must be inside docs/release/evidence/reports"
        )
    if status == "pending":
        if (
            report_value is not None
            or entry.get("sha256") is not None
            or entry.get("executed_at") is not None
        ):
            errors.append(
                f"{evidence_type}: pending evidence must not claim a report, digest, or execution time"
            )
    ...
    if report is None or not report.is_file():
        ...
        if not SHA256.fullmatch(str(entry.get("sha256", ""))) or digest(
            report
        ) != entry.get("sha256"):
        ...
        try:
            ...
            actual = (
                {key: report_document.get(key) for key in expected}
                if isinstance(report_document, dict)
                else {}
            )
            if (
                not isinstance(report_document, dict)
                or report_document.get("schema_version") != "1.0"
            ):
                errors.append(f"{evidence_type}: report schema_version must be 1.0")
            if actual != expected:
                errors.append(
                    f"{evidence_type}: report content does not match the manifest entry"
                )
...
def evidence_errors(
    ...
    *,
    ...
    if not isinstance(manifest, dict) or not RELEASE_VERSION.fullmatch(
        str(manifest.get("release", ""))
    ):
        errors.append("release evidence manifest release must be a semantic version")
    release_identity_value = (
        manifest.get("release_identity") if isinstance(manifest, dict) else None
    )
    release_identity: dict[str, Any] | None = None
    if not isinstance(release_identity_value, dict):
        ...
        release_identity = {
            key: release_identity_value.get(key) for key in RELEASE_IDENTITY_FIELDS
        }
        if set(release_identity_value) != RELEASE_IDENTITY_FIELDS:
            errors.append(
                "release identity must contain exactly the required artifact fields"
            )
        ...
        if source_commit is not None and (
            ...
            errors.append(
                "release identity source_commit must be null or a full lowercase Git SHA"
            )
        ...
        if image_digest is not None and (
            ...
            errors.append(
                "release identity image_digest must be null or a sha256 container digest"
            )
        for field in ("offline_bundle_sha256", "openapi_sha256", "model_set_sha256"):
            ...
            if value is not None and (
                ...
                errors.append(
                    f"release identity {field} must be null or a lowercase SHA-256"
                )
        openapi = root / "docs/openapi.json"
        if (
            openapi.is_file()
            ...
            errors.append(
                "release identity openapi_sha256 does not match docs/openapi.json"
            )
    present: set[str] = set()
    for entry in entries:
        errors.extend(
            validate_entry(entry, release_identity=release_identity, root=root)
        )
    ...
    if pending:
        ...
        missing_identity = sorted(
            key for key, value in release_identity.items() if value is None
        )
        if missing_identity:
            errors.append(
                f"completed release identity is missing: {', '.join(missing_identity)}"
            )
        elif (
            expected_source_commit is not None
            and release_identity["source_commit"] != expected_source_commit
        ):
            errors.append(
                "release identity source_commit does not match the checked-out commit"
            )
    return errors
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Validate Scenara implementation and release evidence"
    )
