...
使用方法:
    python scripts/run_fashion_evaluation.py --dataset tests/fashion_evaluation/dataset.json --output reports/fashion_evaluation_report.json

环境变量:
    SCENARA_FASHION_MODEL_DIR: 模型目录
...
def main() -> None:
    parser = argparse.ArgumentParser(description="运行服饰风格识别质量评估")
    parser.add_argument(
        "--dataset",
        type=Path,
        required=True,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("reports/fashion_evaluation_report.json"),
        help="评估报告输出路径",
    )
    parser.add_argument(
        "--model-dir",
        type=Path,
        help="服饰识别模型目录(覆盖环境变量)",
    )
    parser.add_argument(
        "--no-gpu",
        action="store_true",
    ...
    try:
        ...
        logger.info(
            f"Fashion engine initialized: {engine.model_id} v{engine.version} "
            f"(GPU: {not args.no_gpu})"
        )
    except Exception as exc:
        logger.error(f"Failed to initialize fashion engine: {exc}")
        sys.exit(1)

    # 运行评估
    evaluator = FashionEvaluator(engine)
