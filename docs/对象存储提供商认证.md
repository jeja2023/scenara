...
在容器部署中，将 `SCENARA_S3_PUBLIC_ENDPOINT_URL` 设置为外部客户端可访问的端点。内部端点仍可用于 API 和 worker 流量。Python SDK `upload_asset_direct` 和 TypeScript SDK `uploadAssetDirect` 实现了此工作流。单机内网部署可将 MinIO S3 API 映射到 `59000`，只开放该端口，保持管理控制台端口 `59001` 关闭；MinIO CORS 只允许实际 Core 页面来源。
