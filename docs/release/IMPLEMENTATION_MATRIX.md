...
Current development version: `0.3.0-dev.32` (`0.3.0.dev32` for Python packages).

## Release Gate Status
The `0.3.0-dev.32` engineering baseline enables real vision model capabilities for behavior recognition and fashion analysis, real-time video/stream synchronized canvas bounding box overlays, in-place details drawer, and navigation groupings.
...
`0.3.0-dev.32`. Existing integration, security, backup/restore, and software-license
...
The `0.3.0-dev.32` baseline processes finite video and PDF inputs to EOF unless callers
...
| Check | Result |
|---|---|
| Full Python baseline for `0.3.0-dev.32` | 278 passed, 12 skipped; Ruff, Mypy, OpenAPI/SDK drift, repository contracts, and implementation/development gates pass |
...
| Console regression for `0.3.0-dev.32` | typecheck, production build and 24 unit tests passed; TypeScript SDK checks passed |
| Compose/Kubernetes tag convergence | `docker compose config` and `kubectl kustomize` resolve every application service to `scenara-api:${SCENARA_IMAGE_TAG:-0.3.0-dev.32}` |
