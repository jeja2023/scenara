...
适用版本：`0.3.0-dev.23`。本文是产品边界和演进顺序的权威说明，不是所有产品均已发布的销售清单。
...
适用版本：`0.3.0-dev.23`。当前基线已完成经认证 S3 Provider 抽象、对象不可变发布、SHA-256 完整性校验、生命周期治理和预签名直传，并完成 Core 到 Data 的远程客户端、迁移包和统一 Console 主题整改；生产 Data 服务的导入、备份恢复与切流证据，以及 ANN、语义重排和跨租户联邦仍需后续门禁。
