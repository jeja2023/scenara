...
适用版本：`0.3.0-dev.16`。本文是产品边界和演进顺序的权威说明，不是所有产品均已发布的销售清单。
...
适用版本：`0.3.0-dev.16`。当前基线已将 Index 从“特征存储种子”提升为可执行的索引契约，并交付 Search 的文本、人像结果检索、保存检索、加权排序 profile、重建任务和来源回溯闭环；生产 ANN、语义重排和跨租户联邦仍需后续门禁。
