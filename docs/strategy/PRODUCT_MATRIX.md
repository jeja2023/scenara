...
适用版本：`0.3.0-dev.26`。本文是产品边界和演进顺序的权威说明，不是所有产品均已发布的销售清单。
...
| 产品模块 | Scenara Agent | 可审计的智能动作编排 | 受控种子能力，等待评估与记忆策略 |
...
仓库拓扑通过 `GET /api/v1/platform/repositories` 暴露。当前 `scenara` 是平台集成仓库，已有模型训练仓库映射为 `scenara-model` 专业仓库；`scenara-data` 是由 Core 远程客户端接入的独立服务边界。拓扑契约同时公开职责、排除职责、跨仓库清单/API/事件和禁止共享数据库、禁止跨仓库源码导入等强制规则。四条正式跨仓库契约当前由 `GET /api/v1/platform/contracts` 和 `contracts/repository/v1.0.1/` 发布，历史 `v1.0.0` 目录保持不可变，并由 Schema 摘要、有效示例和向后兼容门禁保护。
...
当前身份底座支持用户名密码登录、平台根令牌和按项目绑定的服务账号 API Key。用户密码仅保存带独立随机盐的 scrypt 摘要，登录成功后签发短期交互式会话，并在登录时解析 Membership/Role 权限。API Key 只在签发时返回一次，服务端仅保存 SHA-256 摘要；子 Key 的 scope 和产品范围不得超过服务账号，撤销、过期、账号停用和租户/项目不匹配都会拒绝认证。项目产品授权可暂停和恢复，媒体、运行、流水线、模型、反馈、Webhooks、企业工作区与 IAM 资源均在共享策略边界执行产品检查。OIDC/SAML/SCIM 外部身份交换仍属于后续门禁。
...
适用版本：`0.3.0-dev.26`。当前基线已完成经认证 S3 Provider 抽象、对象不可变发布、SHA-256 完整性校验、生命周期治理和预签名直传，并完成 Core 到 Data 的远程客户端、迁移包、独立 Data 服务开发闭环、四仓库规范审计、事件信封、结构化可观测性整改和 Qdrant FeatureStore 适配器；生产 Data 服务的导入、备份恢复与切流证据，以及 ANN 真实集群、语义重排和跨租户联邦仍需后续门禁。

## P0-P2 non-model completion

0.3.0-dev.10 已完成剩余的非模型 P0-P2 控制面开发契约，并新增用户名密码登录：项目生命周期请求必须经过可审计的批准或拒绝，审计保留策略支持配置与清理，计费使用量支持幂等计量和席位上限，外部适配器记录支持健康探测。Agent Trace、Evaluation 和带过期时间的 Memory 均在同一租户/项目授权边界内持久化；登录凭据使用带独立盐的 scrypt 摘要，登录会话按 Membership/Role 解析有效 scope。真实模型调用、外部身份签名断言、支付结算和生产 ANN/语义执行仍属于外部服务或真实模型门禁，本版本不伪造其通过。
