...
适用版本：`0.3.0-dev.9`。本文是产品边界和演进顺序的权威说明，不是所有产品均已发布的销售清单。
...
适用版本：`0.3.0-dev.9`。本版本将 Index 从“特征存储种子”提升为可执行的索引契约，并交付 Search 的文本、人像结果检索、保存检索、加权排序 profile、重建任务和来源回溯闭环；生产 ANN、语义重排和跨租户联邦仍需后续门禁。

## P0-P2 non-model completion

0.3.0-dev.9 已完成剩余的非模型 P0-P2 控制面开发契约：项目生命周期请求必须经过可审计的批准或拒绝，审计保留策略支持配置与清理，计费使用量支持幂等计量和席位上限，外部适配器记录支持健康探测。Agent Trace、Evaluation 和带过期时间的 Memory 均在同一租户/项目授权边界内持久化。真实模型调用、外部身份签名断言、支付结算和生产 ANN/语义执行仍属于外部服务或真实模型门禁，本版本不伪造其通过。
