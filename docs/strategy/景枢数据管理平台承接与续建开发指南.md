...
**文档状态：** M0-M7 已实现；M3/M8 生产迁移、恢复和正式切流待验收
**编制日期：** 2026-08-15
...
| 版本 | 日期 | 主要变更 |
| --- | --- | --- |
| 1.0.0 | 2026-08-15 | 建立 Data 新仓承接、迁移、续建和发布门禁指南 |
| 1.1.0 | 2026-08-21 | 在当前仓库增加 `scenara_data` 独立进程、内部 API、幂等和租户隔离契约测试；生产 PostgreSQL/对象存储切流仍由独立 Data 部署负责 |
| 1.2.0 | 2026-08-22 | 对照四个实际仓库回填 M0-M7 实现状态，明确根仓库适配器与独立 Data 事实来源边界 |
...
1. 建立可独立运行的数据管理服务。
2. 消费 `scenara-contracts`，接入 Core 的统一身份、权限、审计和 Console。
...
Data 不建设独立 Console、用户体系、API Key 系统、生产推理、模型训练或人物检索。
...
```mermaid
flowchart LR
    UI["Scenara Console"] --> CORE["Scenara Core API Gateway"]
...
当前四仓库统一消费 Contracts `v1.0.1`；`contracts/repository/v1.0.0` 仅作为不可变历史基线保留，Data 不得私自修改任一已发布 Schema。
