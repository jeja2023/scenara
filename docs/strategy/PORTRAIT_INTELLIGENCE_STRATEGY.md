...
适用版本：`0.3.0-dev.2`。本文是景枢人像 AI 方向的长期技术战略，定义平台演进目标、六大核心模块、三项核心资产以及各阶段的门禁与现状差距。本文是战略意图文件，不是已发布能力清单。能力成熟度以 `model-capabilities.yml` 和 [实现矩阵](../release/IMPLEMENTATION_MATRIX.md) 为准。
...
- **跨摄像头 Re-ID 引擎**：基于 body_embedding + face_embedding 的多模态融合检索
- **人像聚类服务**：无监督聚类 + 人工确认，形成身份图谱节点
- **事件分析**：轨迹关联、行为序列识别、跨场景出现分析
- **持续学习**：难例自动触发重训，模型性能退化自动检测与版本回滚

**当前状态**：约 10%。向量检索（pgvector）提供基础检索能力；聚类、知识图谱、事件分析、持续学习均未建设。

---

## 现状差距评估

本节记录截至 `0.3.0-dev.2` 的实际差距，随版本迭代更新。
