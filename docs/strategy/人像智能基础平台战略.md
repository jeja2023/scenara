...
适用版本：`0.3.0-dev.26`。本文是景枢人像 AI 方向的长期技术战略，定义平台演进目标、六大核心模块、三项核心资产以及各阶段的门禁与现状差距。本文是战略意图文件，不是已发布能力清单。能力成熟度以 `model-capabilities.yml` 和 [实现与验收矩阵](../release/实现与验收矩阵.md) 为准。
...
**平台侧已有**：`HardSampleManifest` 和 `DatasetVersionReference` 契约（[contracts/repository/v1.0.0/](../../contracts/repository/v1.0.0/)）；Core 的远程 Data 客户端、带校验和的迁移导出和切流门禁（见 [Data 切流操作说明](./数据平台切流操作说明.md)）。
...
**平台侧已有**：模型准入 API（`POST /api/v1/model-packages/admissions`）、发布状态机（`candidate → validated → approved → active → retired`）、`ModelDeploymentEvent` Webhook（[仓库拓扑](./仓库拓扑.md)）。
...
**综合评估**：平台底座（IAM、媒体契约、模型准入状态机、特征存储抽象）已基本到位，这是正确的建设顺序。当前最紧迫的缺口是**人像算法模型制品**——在 SCRFD + ArcFace ONNX 就绪之前，平台无法对外宣称具备完整人像识别能力。数据治理和 Foundation Model 是 1.0 之后的建设目标，按 [产品矩阵](./产品矩阵.md) 的升级原则推进。
...
**触发条件**：独立 `scenara-data` 服务完成迁移导入、影子读、备份恢复、Hard Sample 幂等接收和回滚演练（见 [Data 切流操作说明](./数据平台切流操作说明.md)）。
...
| 文件 | 与本文的关系 |
| --- | --- |
| [产品矩阵](./产品矩阵.md) | 产品模块状态与升级原则，本文的 AI 战略是其中 Model / Data / Parse 模块的深化 |
| [仓库拓扑](./仓库拓扑.md) | 三仓库分工与跨仓库契约，本文遵从其边界约束 |
| [访问底座](./访问底座.md) | IAM 与认证体系，本文所有产品模块共享 |
| [实现矩阵](../release/实现与验收矩阵.md) | 能力成熟度实时状态，优先级高于本文的定性描述 |
...
- `modules` — 六个战略模块，每项声明 `maturity`、`owner_repository_id`、`current_scope`、`not_in_scope_yet` 与 `next_gate`。`owner_repository_id` 与[仓库拓扑](./仓库拓扑.md)一致：数据治理与标注平台归 `scenara-data`，模型训练与人像算法制品归 `scenara-model`，向量检索与模型运维归 `scenara`。
