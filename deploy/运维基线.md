...
本文档定义单节点、多 GPU 私有化部署的升级、回滚、探针、指标和告警最低要求。当前工程基线为 `0.3.0-dev.43`；它不能替代目标环境的可复现演练报告。
...
- API 默认仅绑定回环地址，由受管 TLS 反向代理提供证书续期、请求体上限、连接/速率限制和访问日志。应用会在读取 multipart 请求体前按 `SCENARA_MAX_MULTIPART_UPLOAD_BYTES` 拒绝超限直传，并在流式写入时再次复核；网关的 `client_max_body_size` 必须不大于该值。更大的媒体只能走经 SHA-256 核验的预签名上传。`SCENARA_ALLOWED_HOSTS` 与 `SCENARA_FORWARDED_ALLOW_IPS` 必须精确到实际域名和代理地址，禁止 `*`。
- PostgreSQL 连接预算以所有 API、worker、scheduler 副本乘以 `SCENARA_POSTGRES_POOL_MAX_SIZE` 计算，并额外预留迁移、监控和运维连接。默认池为每进程 `1..4`；扩大副本或池之前必须完成并发压测，接近数据库上限时使用 PgBouncer。
...
- `/metrics` 输出 Prometheus 文本并沿用 API 认证策略，生产抓取任务必须携带 Bearer Token；必须采集 Redis batch/stream 的 backlog 与 pending，并以 backlog 驱动 worker 容量决策。
...
- 支持 Ubuntu 24.04 x86_64、Docker Compose 和一张或多张可被 `nvidia-smi` 测量的 NVIDIA GPU；生产 Compose 为每个 GPU worker 保留一张 GPU，显存容量记录在资格报告中，但不设固定 GPU 数量或显存门槛。
