...
CONTEXT = PrincipalContext(tenant_id="default", project_id="default", principal_id="surveillance-test")
...
async def _prepared_task(runtime):
    identity = await runtime.portrait.create_identity(CONTEXT, CreateIdentityRequest(display_name="目标人员"))
    ...
    await runtime.trajectory.register_camera(
        CONTEXT, RegisterCameraRequest(camera_id="camera-surveillance", display_name="测试点位")
    )
    ...
    member = await runtime.surveillance.create_member(
        ...
        CreateWatchlistMemberRequest(portrait_identity_id=identity.identity_id, display_label="目标人员"),
    )
    task = await runtime.surveillance.create_task(
        ...
        CreateSurveillanceTaskRequest(
            ...
            bindings=[TaskBinding(binding_id="bind-surveillance", source_id=source.source_id, camera_id="camera-surveillance")],
            threshold_policy=ThresholdPolicy(policy_version="test-v1", face_threshold=0.8, body_threshold=None),
            cooldown_seconds=30,
        ),
    )
...
@pytest.mark.asyncio
async def test_watchlist_reuses_portrait_identity_and_alert_debounce(development_settings) -> None:
    ...
    try:
        ...
        await runtime.surveillance._evaluate_observation(CONTEXT, task, binding, observation)
        await runtime.surveillance._evaluate_observation(CONTEXT, task, binding, observation)
        ...
        deliveries = await runtime.state.list_webhook_deliveries(CONTEXT.tenant_id, CONTEXT.project_id, 10)
        assert [(item.event_type, item.event_id) for item in deliveries] == [("alert.triggered", events[0].event_id)]
...
@pytest.mark.asyncio
async def test_alert_triage_is_revision_protected_and_replayed(development_settings) -> None:
    runtime = await _runtime(development_settings)
    try:
        ...
        alert = (await runtime.surveillance.list_alerts(
            CONTEXT,
            status=None,
            task_id=None,
            camera_id=None,
        ...
        triaged = await runtime.surveillance.triage_alert(
            CONTEXT,
            alert.alert_id,
            TriageAlertRequest(expected_revision=alert.revision, status="false_positive", reason="人工核验不一致"),
        )
        ...
        with pytest.raises(Exception, match="triaged|revision"):
            await runtime.surveillance.triage_alert(
                CONTEXT,
                alert.alert_id,
                TriageAlertRequest(expected_revision=alert.revision, status="ignored", reason="重复操作"),
            )
        events = await runtime.surveillance.events_after(CONTEXT, 0)
        assert [event.event_type for event in events] == ["alert.triggered", "alert.triaged"]
...
@pytest.mark.asyncio
async def test_surveillance_watchlist_api_uses_standard_envelopes(development_settings) -> None:
