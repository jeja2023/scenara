...
- Primary product surfaces: Scenara Console 的 12 个工作区、全局导航、连接设置和管理对话框。
- Evidence reviewed: `README.md`、`docs/brand/BRAND.md`、`frontend/console/src/App.vue`、`frontend/console/src/router.ts`、`frontend/console/src/styles.css`、`frontend/console/src/labels.ts`、全部 `frontend/console/src/views/*.vue`、Vitest 与 Playwright 验收用例。
...
- Goals: 让中文企业用户不依赖英文知识即可完成日常操作；统一展示产品矩阵、视觉解析、模型治理、IAM、运维与企业治理能力。
- Non-goals: 不翻译品牌名、协议名、算法名、标准单位和必须原样输入的技术标识；不在本轮改变 API 契约或业务流程。
- Success signals: 12 个工作区的通用界面文案全部为中文；枚举和后端英文说明不直接暴露；桌面与移动端无溢出或交互回归。

## Personas and jobs

- Primary personas: 平台管理员、算法与集成工程师、运维人员、审核与合规人员。
- User jobs: 管理媒体和运行、查看结果、配置流水线与模型、管理访问凭据和产品授权、审核反馈、检查运行状态与合规证据。
- Key contexts of use: 企业内网、私有化部署、桌面高频操作和移动端临时查看。

## Information architecture

- Primary navigation: 工作台、领域、配置、治理四组导航。
- Core routes/screens: 总览、媒体、运行、结果、人像解析、OCR 文档、流水线、模型、反馈与发布、接入、运维、企业。
...
- New/changed components: 扩充 `labels.ts` 为统一界面翻译层；不新增独立设计系统。
...
- Success: 一次性密钥、复制和保存结果使用明确中文确认。
- Disabled: 按钮禁用时保留可理解的中文上下文或提示。
- Offline/slow network: 全局连接状态使用“检查中 / 已连接 / 未连接”。
...
- Performance constraints: 标签映射为本地常量与纯函数，不增加网络请求和运行时依赖。
- Compatibility constraints: API 请求与响应字段、资源标识、协议值保持不变；仅改变用户可见展示。
- Test/screenshot expectations: Vitest 静态扫描阻止通用英文回归；Playwright 覆盖 12 个路由、桌面和移动视口、标签页及一次性凭据流程。
